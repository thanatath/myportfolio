    ...
    return (
      ...
      >
        ...
        <div className="flex justify-center md:gap-8 mb-[25px]">
          <span>
            <FontAwesomeIcon className="mx-2" icon={solid("envelope")} />
            {email}
          </span>
          <span>
            <FontAwesomeIcon className="mx-2" icon={solid("phone")} />
            {phone}
          </span>
        </div>
        ...
        <div className="flex text-center w-full mx-auto mb-[25px] gap-8 justify-center ">
          <button
            style={{ display: !cv || cv.trim() === "" ? "none" : "block" }}
            disabled={!cv || cv.trim() === ""}
            className="w-[150px] shadow-md hover:bg-[white] hover:text-[black] ease-in-out duration-300 h-[50px] rounded-[15px] border-2"
          >
